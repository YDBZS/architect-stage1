...
public class ShopcatController {
    ...
    public ReturnResult add(
        ...
        String shopcartJson = redisOperator.get(Constant.FOODIE_SHOPCART + ":" + userId);
        ...
        // 覆盖现有redis中的购物车
        redisOperator.set(Constant.FOODIE_SHOPCART + ":" + userId, JsonUtils.objectToJson(shopcartList));
    ...
    public ReturnResult del(
        ...
        // 用户在页面删除购物车中的商品数据，如果此时用户已经登录，则需要同步删除Redis中购物车中的商品
        String shopcartJson = redisOperator.get(Constant.FOODIE_SHOPCART + ":" + userId);
        if (StringUtils.isNotBlank(shopcartJson)) {
            // redis中已经有购物车了
